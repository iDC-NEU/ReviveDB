#include "tpcc_hash_index_dash.h"
#include "nvm_init.h"
...
namespace TPCC_DASH_INDEX {

inline RowId UniqueSearch(Transaction *tx, uint64 key, void* index, Table *tbl, RAMTuple *res)
{
    RowId rowId = dash_find(index, key);
...
private:
    ...
    static_assert((uint64_t(15) << 60) >> 60 == 15);
    static uint64 pack(uint64_t key, uint64_t index_idx)
    {
        return key + (index_idx << 60);
    }
...
public:
    ...
    void InitBench()
    {
        ...
        std::string dash_file = "/mnt/pmem0/bench10";
        if (is_init) {
            std::remove(dash_file.c_str());
            init(dash_file.c_str());
            InitDB(dir_config);
            idxs = dash_create();
        } else {
    ...
    inline bool SelectTuple(Transaction *tx, TableType table_type, uint64 key, RAMTuple *tuple, RowId *row_id = nullptr,
                            bool secondary = false)
    {
        // 只有 customer 需要插入secondary index
        RowId rowId;
        if (secondary) {
            rowId = UniqueSearch(tx, pack(key, TABLE_OFFSET(TableType::TABLE_INVALID)), idxs,
                                 tables[TABLE_OFFSET(table_type)], tuple);
        } else {
            rowId =
                UniqueSearch(tx, pack(key, TABLE_OFFSET(table_type)), idxs, tables[TABLE_OFFSET(table_type)], tuple);
        }
    ...
    /* Assume you have inited and prepared already. */
    inline void InsertTupleWithIndex(Transaction *tx, TableType table_type, uint64 key, RAMTuple *tuple,
                                     uint64 *seckey = nullptr)
    {
        ...
        /* insert table index */
        dash_insert(idxs, pack(key, TABLE_OFFSET(table_type)), rowId);
        /* insert secondary index if need */
        if (table_type == TABLE_CUSTOMER) {
            dash_insert(idxs, pack(*seckey, TABLE_OFFSET(TableType::TABLE_INVALID)), rowId);
        }
    }
...
class TPCCHashDashTest : public ::testing::Test {
protected:
    void SetUp() override
    {}
...
TEST_F(TPCCHashDashTest, TPCCTestMain)
{
...
// backup
// cp -r /mnt/pmem0/bench2 /mnt/pmem0/bench3& cp -r /mnt/pmem1/bench2 /mnt/pmem1/bench3& cp -r /mnt/pmem2/bench2
// /mnt/pmem2/bench3& cp -r /mnt/pmem3/bench2 /mnt/pmem3/bench3 restore rm -rf /mnt/pmem0/bench2 /mnt/pmem1/bench2
// /mnt/pmem2/bench2 /mnt/pmem3/bench2 cp -r /mnt/pmem0/bench3 /mnt/pmem0/bench2& cp -r /mnt/pmem1/bench3
// /mnt/pmem1/bench2& cp -r /mnt/pmem2/bench3 /mnt/pmem2/bench2& cp -r /mnt/pmem3/bench3 /mnt/pmem3/bench2
